...
restService.post('/hook', function(req, res) {
  ...
  retrieveUser(req, (error, user) => {
    if(error) {
      res.status(500).json({error: 'Unable to retrieve user'})
    }
    const day = req.body.result.parameters.date
    const speech =  retrieveLogFor(user, day, (error, speech) => {
      return res.json({
        speech: speech,
        displayText: speech,
...
function retrieveUser(req, cb){
  ...
  const source = req.body.originalRequest.source
  ...
  slack.retrieveUsername(req.body.originalRequest.data.user, (error, user) => {
    if(error) {
